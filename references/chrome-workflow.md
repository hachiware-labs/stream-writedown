...
`Language` は本文の再構成前に確定し、inventory の `language_checked: true` とともに保存する。英語なら日本語訳工程へ送る。言語固有処理が失敗した場合、当該項目だけ理由付き `partial` として inventory へ直ちに保存し、同じバッチで既に検証済みの項目を失わない。
